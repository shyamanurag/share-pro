...
// This API endpoint seeds the database with Indian NSE stocks
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    return res.status(200).json({ 
      message: 'Indian NSE stocks seeded successfully',
      stocks 
    });
  } catch (error) {
    console.error('Error seeding Indian stocks:', error);
    return res.status(500).json({ error: 'Failed to seed Indian stocks' });
  }
}